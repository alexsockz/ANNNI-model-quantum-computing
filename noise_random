...
vec_rand = np.zeros(limite)
def qcnn_ansatz_noisy(num_qubits, params, r):
    ...
    # Convolution block
    def conv(wires, params, index, pos, r):
        if len(wires) % 2 == 0:
            ...
            if answer == "y":
                if r[pos] < noise_strength / 3:
                    X(wires=int(wires[-1]))
                elif noise_strength / 3 < r[pos] < 2 * noise_strength / 3:
                    Y(wires=int(wires[-1]))
                elif 2 * noise_strength / 3 < r[pos] < 3 * noise_strength / 3:
                    Z(wires=int(wires[-1]))
            pos += 1
            index += 1
    ...
    # Pooiling block
    def pool(wires, params, index, pos, r):
        for wire_pool, wire in zip(wires[0::2], wires[1::2]):
            ...
            if answer == "y":
                if r[pos] < noise_strength / 3:
                    qml.cond(m_0 == 0, X)(wires=int(wire))
                elif noise_strength / 3 < r[pos] < 2 * noise_strength / 3:
                    qml.cond(m_0 == 0, Y)(wires=int(wire))
                elif 2 * noise_strength / 3 < r[pos] < 3 * noise_strength / 3:
                    qml.cond(m_0 == 0, Z)(wires=int(wire))
            pos += 1
            qml.cond(m_0 == 1, qml.RX)(params[index + 1], wires=int(wire))
            # Dopo le RX condizionali, aggiungiamo rumore sui qubit ancora attivi
            if answer == "y":
                if r[pos] < noise_strength / 3:
                    qml.cond(m_0 == 1, X)(wires=int(wire))
                elif noise_strength / 3 < r[pos] < 2 * noise_strength / 3:
                    qml.cond(m_0 == 1, Y)(wires=int(wire))
                elif 2 * noise_strength / 3 < r[pos] < 3 * noise_strength / 3:
                    qml.cond(m_0 == 1, Z)(wires=int(wire))
            pos += 1
            index += 2
    ...
    while len(active_wires) > 2:
        index = conv(active_wires, params, index, pos, r)
        index, active_wires = pool(active_wires, params, index, pos, r)
        qml.Barrier()
...
num_params, output_wires = qcnn_ansatz_noisy(num_qubits, [0]*100, [0]*100)



@qml.qnode(qml.device("default.qubit", wires=num_qubits))
def qcnn_noisy(params, state, r):
    qml.StatePrep(state, wires=range(num_qubits), normalize=True)
    _, output_wires = qcnn_ansatz_noisy(num_qubits, params, r)
    return qml.probs([int(k) for k in output_wires])

# Vectorized circuit through vmap
vectorized_qcnn_noisy = vmap(jit(qcnn_noisy), in_axes=(None, 0, None))
...
def train_qcnn(num_epochs, lr, T, seed):
    ...
    def loss_fun(params, X, Y):
        ...
        for _ in range(limite):
            vec_rand = np.random.rand(100)
            preds = vectorized_qcnn_noisy(params, X, vec_rand) + preds
        preds = preds / limite
...
def predis(params, state):
    preds = 0
    for _ in range(limite):
        vec_rand = np.random.rand(100)
        preds = vectorized_qcnn_noisy(params, state, vec_rand) + preds
    preds = preds / limite
    return preds

predicted_classes = np.argmax(
    predis(trained_params, psis.reshape(-1, 2**num_qubits)),
    axis=1
)

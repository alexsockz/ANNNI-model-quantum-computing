...
if answer == "y":
    def anomaly_noisy_scaled(n_qubit, params, scale):
        """Ansatz of the QAD model
        Apply multi-qubit gates between trash and non-trash wires
        """
        ...
        def block(nontrash, trash, shift):
            ...
            for i, wire in enumerate(trash):
                ...
                if answer == "y":
                    ...
                    qml.DepolarizingChannel(noise_strength * scale, wires=int(target))
            # Connect each nontrash wire to a trash wire
            for i, wire in enumerate(nontrash):
                trash_idx = (i + shift) % len(trash)
                qml.CNOT(wires=[int(wire), int(trash[trash_idx])])
                if answer == "y":
                    qml.DepolarizingChannel(noise_strength * scale, wires=int(wire))
                    qml.DepolarizingChannel(noise_strength * scale, wires=int(trash[trash_idx]))
    ...
    def extrapolate_points(exps_list, scales):
        ...
        Restituisce exp mitigate di forma (N, C).
        """
        # exps_list è una tupla con 21 array
        n_scales = len(exps_list)
        exps_stack = np.array(exps_list)  # shape: (21, N, n_trash)
    ...
    def make_vectorized_circuit(scale):
        @jax.jit
        def circ(params, state):
            return anomaly_with_scaled_noise(params, state, scale)
